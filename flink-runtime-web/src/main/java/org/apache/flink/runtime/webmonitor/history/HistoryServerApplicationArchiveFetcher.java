...
public class HistoryServerApplicationArchiveFetcher<Entry>
    ...
    private final Map<String, ArchiveMetaInfo> applicationArchiveMetaInfoCache;
    ...
    List<ArchiveEvent> lazyProcessArchive(String archiveId, Path archivePath, Path refreshDir)
        ...
        if (!applicationArchiveMetaInfoCache.containsKey(archiveId)) {
            ...
            events.add(
                    new ArchiveEvent(
                            archiveId,
                            applicationArchiveMetaInfoCache.get(archiveId).getEventType()));
            return events;
        }

        events.addAll(processArchive(archiveId, archivePath, refreshDir, LAZY));

        applicationArchiveMetaInfoCache.get(archiveId).setEventType(ArchiveEventType.CREATED);
        return events;
    }
