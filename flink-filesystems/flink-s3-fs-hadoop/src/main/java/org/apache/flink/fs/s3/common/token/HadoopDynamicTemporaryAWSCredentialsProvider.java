 ...
 *
 * <p>This credential provider must not fail during construction, because that would break the
 * chain of credential providers it is registered in. It is, however, expected to fail fast from
 * {@link #resolveCredentials()} by throwing {@link NoAwsCredentialsException} when no delegation
 * token credentials are available yet; the surrounding {@code AwsCredentialsProviderChain} treats
 * that as a signal to move on to the next provider in the chain.
 */
@Internal
