 ...
 *
 * <p><b>Cardinality.</b> {@code op} comes from the SDK operation name (a closed set of ~15 values
 * for S3), {@code status_class} is a closed classifier ({@code 2xx}, {@code 4xx}, {@code 5xx},
 * {@code throttled}, {@code other}, {@code error}, {@code unknown}), and {@code reason} is a closed
 * enum ({@code throttled}, {@code 5xx}, {@code other}). Metric handles are cached in bounded maps,
 * so {@link #publish} is a map lookup plus a counter increment with no per-record allocation.
 *
